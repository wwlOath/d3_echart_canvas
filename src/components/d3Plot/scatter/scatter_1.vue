        ...
        <div v-show="isShowTsne">
          <div class="tsne-content">
            <el-row class="operate-btn">
               <i class="el-icon-download" @click="downloadImage('tsneSample')"></i>
            </el-row>
            <div id="tsneSample" :style="containerStyle">
              <div id="tsneSample_plotRegion" ref="tsneSampleRef">
                ...
                <!--右侧legend-->
                <div style="position: absolute;right: 0;" v-if="markTag">
                  <ul style="list-style-type: none; margin: 0px; padding: 0px; text-align: left;">
                    <li @click="showOrHideCell(key)" style="cursor: pointer; margin-bottom: 5px; margin-right: 24px; display: block;" v-for="(value, key, index) in set_cluster_transfrom"  :style="{display: index >= (currentPage - 1) * pageSize && index < currentPage * pageSize ? '' : 'none'}">
                      <i :style="{'background-color': value ? set_color_cluster(key) : 'rgba(200,200,200,0.8)'}" style="width: 9px; height: 9px; display: inline-block; margin-right: 4px; vertical-align: middle;border-radius: 50%"></i>
...
<style lang="less">
  ...
  #tsneSample_plotRegion, #tsneExpression_plotRegion {
    margin: auto;
    width: 100%;
    height: 600px;
  }
...
<script>
  ...
  export default {
    ...
    methods: {
      showOrHideCell(val) {
        this.markTag = false;
        let data = this.set_cluster_transfrom;
        let mark = data[val];
        mark = !mark;
        this.tsneUmapDataList.points.forEach((d) => {
          if(d[2] == val) {
            if(mark) {
              d[8] = true;
            }else{
        ...
        this.set_cluster_transfrom[val] = mark;
      ...
      getTsneDataList() {
        ...
        return axios.get('../../../../static/_web_total_stat_for_web/umap_1.json').then(res => {
          ...
          this.umapLabels = this.set_cluster_label;
           ...
           this.set_cluster.forEach((d) => {
             this.set_cluster_transfrom[d] = true;
          })
        })
      },
