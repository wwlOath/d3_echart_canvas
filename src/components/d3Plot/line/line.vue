    ...
    methods: {
      getData() {
        ...
        chart.scaleX = d3.scaleTime()
            .domain(d3.extent(dataOrigin, function(d) { return d3.timeParse("%Y-%m-%d")(d.date); }))
            .range([ 0, chart.getBodyWidth()]);

        chart.scaleY =  d3.scaleLinear()
            .domain([0, d3.max(dataOrigin, function(d) { return +d.value; })])
            .range([ chart.getBodyHeight(), 0 ]);

        /* ----------------------------渲染柱形------------------------  */
        chart.renderBars = function () {
          let groups = chart.body();
          ...
          groups.append("path")
              ...
              .attr("d", d3.line()
                  .x(function(d) { return chart.scaleX(d3.timeParse("%Y-%m-%d")(d.date)) })
                  .y(function(d) { return chart.scaleY(d.value) })
              )
        }
